...
def correlation (dataset, x,y="rate shifted - rate swapped (NN)",diatonic="include", normalize=True, figsize=(6,6), type='pearson', itirations = 10000, show_stats=True, width=8, height=6, save_to=None, **kwargs):
    temp = dataset #loading dataset
    ...
    sns.regplot(x=x, y=y, data=temp, ax=ax, **kwargs)
    if show_stats:
...
def ridge_coeffs(dataset, X_vars,y_vars,diatonic="include"):
    ...
    coefs, P_Vals, P_Val_fullmodel = permtest_coeffs(X_vars=X_vars, y_vars=y_vars, full_score= np.sqrt(score), coefficients=coefficients, dataset=dataset)
    print("p-values:")
    print(P_Vals)
    print("p-values full model:")
    print(P_Val_fullmodel)
    print("DONE.")
